...
//   const wheelCooldownMs = 450; // prevent over-triggering while animating
...
//           {/* Header */}
//           <div className="text-center mb-16">
//             <h2 className="text-4xl md:text-5xl font-bold text-foreground mb-6">Client Testimonials</h2>
//             <p className="text-xl text-muted-foreground max-w-3xl mx-auto">
//               Hear from leaders who have transformed their organizations with our HR expertise.
//             </p>
//           </div>
...
//             {/* Stage (click left = prev, right = next) */}
//             <div
//               className="relative h-[430px] md:h-[400px] lg:h-[380px] overflow-visible cursor-pointer select-none"
//               role="button"
//               tabIndex={0}
//               aria-label="Testimonials carousel. Click right for next, left for previous. Use arrow keys as well."
//               onClick={(e) => {
//                 if (!canNavigate) return;
...
//               }}
//               onTouchMove={(e) => {
//                 if (!canNavigate || !touchActive.current) return;
//                 // If vertical movement is dominant, cancel swipe handling
//                 const t = e.touches[0];
...
//                   return;
//                 }
//                 // prevent accidental multi-trigger while moving; we'll act on touchend
//               }}
//               onTouchEnd={(e) => {
//                 if (!canNavigate || !touchActive.current) return;
...
//                 if (!canNavigate) return;
//                 const now = Date.now();
//                 if (now - lastWheelTime.current < wheelCooldownMs) return;
//                 // Prefer horizontal wheel delta, fallback to vertical (trackpads)
...
//                 // Wrap-around for nicer neighbor selection
//                 const shortest =
//                   ((delta + count + Math.floor(count / 2)) % count) - Math.floor(count / 2);
//                 const isCenter = shortest === 0;
//                 const isNeighbor = Math.abs(shortest) === 1;
//                 const translateXPercent = shortest * 110; // spacing between cards
//                 const scale = isCenter ? 1 : isNeighbor ? 0.96 : 0.9;
//                 const translateY = isCenter ? 0 : isNeighbor ? 8 : 16;
//                 const z = isCenter ? 30 : isNeighbor ? 20 : 10;
//                 const opacity = count <= 1 ? 1 : isCenter ? 1 : isNeighbor ? 1 : 0.0;

//                 return (
//                   <div
//                     key={index}
//                     className="absolute left-1/2 top-0 max-w-screen h-full"
//                     style={{
//                       transform: `translateX(${translateXPercent}%) translateX(-50%) translateY(${translateY}px) scale(${scale})`,
...
//                       if (count <= 1) return;
//                       const delta = index - current;
//                       const shortest = ((delta + count + Math.floor(count / 2)) % count) - Math.floor(count / 2);
//                       if (shortest < 0) {
//                         goPrev();
//                       } else if (shortest > 0) {
//                         goNext();
//                       } 
//                       // Don't navigate if it's the center testimonial (shortest === 0)
//                     }}
//                     role="button"
//                     tabIndex={-1}
//                   >
//                     <div
//                       className={`bg-white border-b border-primary/10 rounded-xl h-[415px] flex flex-wrap p-8 transition-all duration-300 relative ${isCenter ? "shadow-elegant" : "shadow-elegant/40"}`}
//                     >
//                       {/* Quote Icon */}
//                       <div className="absolute -top-4 left-8">
//                         <div className="w-8 h-8 bg-primary rounded-full flex items-center justify-center">
...
//                       {/* Author */}
//                       <div className="flex items-center space-x-4">
//                         <div className="w-12 h-12 bg-primary/10 rounded-full flex items-center justify-center text-2xl overflow-hidden">
//                           {testimonial?.image ? (
//                             <img
//                               src={testimonial.image}
